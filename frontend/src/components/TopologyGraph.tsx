    ...
    <div className="glass-panel p-6 rounded-2xl theme-border border space-y-5">
      ...
      <div className="flex items-center justify-between border-b pb-4 theme-border">
        <div className="space-y-0.5">
          ...
          <h2 className="text-lg font-bold text-title tracking-tight">Infrastructure Service Cluster</h2>
        </div>
      ...
      {/* Pipeline Data Trace */}
      <div className="card-bg-subtle p-3 rounded-xl border theme-border text-xs font-mono text-subtitle flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="px-2 py-0.5 rounded bg-blue-600 text-white font-bold text-[10px] uppercase tracking-wider">
            Trace Path
          </span>
          <span className="font-bold text-title">Production Request Pipeline</span>
        </div>
        <div className="flex items-center gap-1.5 text-[11px] text-subtitle font-mono">
          <span>Nginx (8080)</span>
          <ArrowRight className="w-3 h-3 text-blue-500 shrink-0" />
          <span>Node API (3000)</span>
          <ArrowRight className="w-3 h-3 text-blue-500 shrink-0" />
          <span>PostgreSQL (5432) & Redis (6379)</span>
        </div>
      </div>
    </div>
  );
