...
export const CommandCenter: React.FC<CommandCenterProps> = ({ incidents, onRefreshIncidents }) => {
  const [promptText, setPromptText] = useState<string>('Meri production API down hai, 502 Bad Gateway aa raha hai. Root cause trace karke fix karo.');
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 text-xs font-semibold">
                OpenAI Agentic Orchestrator
              </span>
              <span className="text-xs font-mono text-slate-400">Model: GPT-4o Tool Calling</span>
            </div>
            <h1 className="text-2xl font-extrabold text-white mt-1">Incident Command Center</h1>
            <p className="text-xs text-slate-400 mt-1">
              Natural language prompt input for production issues. OpenAI agent inspects container status, Nginx logs, stack traces, and commit diffs.
            </p>
          </div>
          ...
          <div className="space-y-2">
            {incidents.length === 0 ? (
              <div className="p-6 text-center text-xs text-slate-500 glass-panel rounded-xl">
                No active incidents. Click "Investigate Outage" above.
              </div>
            ) : (
              incidents.map(inc => {
